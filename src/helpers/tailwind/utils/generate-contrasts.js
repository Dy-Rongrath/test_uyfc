...
const generateContrasts = (palette) =>
{
    ...
    {
        darkColor = contrast(color, '#FFFFFF') > contrast(darkColor, '#FFFFFF') ? color : darkColor;
    }));

    // Generate the contrasting colors
    return fromPairs(map(palette, ((color, hue) => [
            hue,
            contrast(color, darkColor) > contrast(color, lightColor) ? darkColor : lightColor,
        ]
    )));
};
