...
// -----------------------------------------------------------------------------------------------------
// @ Helpers TailwindCSS Main Plugin
// -----------------------------------------------------------------------------------------------------
const theming = withOptions((options) => ({
    ...
    }) =>
    {
        ...
        /* Get the file path */
        const filename = resolve(__dirname, ('../../styles/user-themes.scss'));
        ...
        addComponents(
            fromPairs(map(options.themes, (theme, themeName) => [
                ...
                ))), (value, key) => [[`--helpers-${e(key)}`, value], [`--helpers-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
            ])),
        );
        ...
        const schemeCustomProps = map(['light', 'dark'], (colorScheme) =>
        {
            ...
            const background = theme(`helpers.customProps.background.${colorScheme}`);
            const foreground = theme(`helpers.customProps.foreground.${colorScheme}`);
            ...
            return {
                [(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
                    ...
                    ...fromPairs(flatten(map(background, (value, key) => [[`--helpers-${e(key)}`, value], [`--helpers-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                    ...fromPairs(flatten(map(foreground, (value, key) => [[`--helpers-${e(key)}`, value], [`--helpers-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                },
            };
        });
            ...
            theme: {
                extend: {
                    ...
                    colors: fromPairs(flatten(map(keys(flattenColorPalette(normalizeTheme(options.themes.default))), (name) => [
                        [name, `rgba(var(--helpers-${name}-rgb), <alpha-value>)`],
                        [`on-${name}`, `rgba(var(--helpers-on-${name}-rgb), <alpha-value>)`],
                    ]))),
                },
                helpers  : {
                    customProps: {
