...
import { ScrollbarGeometry, ScrollbarPosition } from 'helpers/directives/scrollbar/interface';
 ...
 */
@Directive({
    selector  : '[helpersScrollbar]',
    exportAs  : 'helpersScrollbar',
    standalone: true,
})
export class HelpersScrollbarDirective implements OnChanges, OnInit, OnDestroy
{
    /* eslint-disable @typescript-eslint/naming-convention */
    static ngAcceptInputType_helpersScrollbar: BooleanInput;
    /* eslint-enable @typescript-eslint/naming-convention */

    @Input() helpersScrollbar: boolean = true;
    @Input() helpersScrollbarOptions: PerfectScrollbar.Options;
    ...
    {
        // Enabled
        if ( 'helpersScrollbar' in changes )
        {
            // Interpret empty string as 'true'
            this.helpersScrollbar = coerceBooleanProperty(changes.helpersScrollbar.currentValue);

            // If enabled, init the directive
            if ( this.helpersScrollbar )
            {
                this._init();
            }
        ...
        // Scrollbar options
        if ( 'helpersScrollbarOptions' in changes )
        {
            // Merge the options
            this._options = merge({}, this._options, changes.helpersScrollbarOptions.currentValue);
